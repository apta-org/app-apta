...
const prepareFinalRules = (courses) => {
  ...
  // Associate each rule with a valid course
  ...
  // Find required course marks for each rule and update that rule
  tmpFinalRules.forEach((rule) => {
    let matchingRequiredCourseFound = false
    courses.forEach((course) => {
      if (!matchingRequiredCourseFound && (rule.requiredCourseDescription.startsWith(course.description) ||
      rule.requiredCourseDescription.startsWith(course.name))) {
        rule.qualifiedMarks = course.minimumMarks
        finalRules.push(rule)
        matchingRequiredCourseFound = true
      }
    })
  })
}
