...
const CourseJSON = Joi.object().keys({
  ...
  minimumMarks: Joi.number().required().description('The course minimumMarks required'),
  allowedForProgram: Joi.boolean().required().description('The course eligibility status')
})
...
const ErrorOutputValidation = Joi.object().keys({
  errors: Joi.object().keys({
    ...
    minimumMarks: Joi.array().items(Joi.string()).optional(),
    allowedForProgram: Joi.array().items(Joi.string()).optional()
  })
})
